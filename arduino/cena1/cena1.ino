...
// DEFINIÇÕES SENSOR HC-SR04: >> VCC (+5V) / GND (0V) / ECHO (digital) / TRIG
// (digital)
...
// DEFINIÇÕES DO DISPLAY: >> EN, RS, DB4, DB5, DB6, DB7 >> PSB (HIGH) >> R/W
// (LOW)
...
Keypad teclado = Keypad(makeKeymap(TECLAS_MATRIZ), PINOS_LINHAS, PINOS_COLUNAS,
                        LINHAS, COLUNAS);
...
{
  lcd.setCursor(a, b); // Posiciona o cursor na Coluna a x Linha b >> L1(0,0),
                       // L2(0,2), L3(8,0), L4(8,2)
  lcd.print(texto);    // Imprime o texto na posição (a,b) do LCD >> Tamanho
                       // máximo: 16 caracteres/linha
}
...
{
  lcd.clear();
  lcd.setCursor(0, 0); // Posiciona o cursor na Coluna a x Linha b >> L1(0,0),
                       // L2(0,2), L3(8,0), L4(8,2)
  char x = 0x08;
  for (int i = 0; i < 32; i++)
  {
    lcd.write(x); // Imprime o texto na posição (a,b) do LCD >> Tamanho máximo:
                  // 16 caracteres/linha
  }
  lcd.setCursor(0, 2); // Posiciona o cursor na Coluna a x Linha b >> L1(0,0),
                       // L2(0,2), L3(8,0), L4(8,2)
  for (int i = 0; i < 32; i++)
  {
    lcd.write(x); // Imprime o texto na posição (a,b) do LCD >> Tamanho máximo:
...
void loop()
{
  while (controle == false)
  {
    ...
    float distancia =
        sonar.ping_cm();       // Realiza a captura da distancia em [cm].
    Serial.println(distancia); // Imprime na serial o valor da variável
                               // distancia em [cm].
    delay(500);
    if (distancia < 60)
    {
